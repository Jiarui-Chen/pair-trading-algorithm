...
class PairSelector():
    ...
    def extract_price_data(self, symbols):
        qb = QuantBook()
        tickers = [qb.AddEquity(t, dataNormalizationMode=DataNormalizationMode.Adjusted).Symbol for t in symbols]
        history = qb.History(tickers, self.start_date, self.end_date, Resolution.Daily)
    ...
    def calc_return(self, history):
        start, end = 0, 0
        history["return"] = pd.Series()
        while end < len(history): # Ending condition: reach the end of the dataframe
            if history.iloc[start, 0] == history.iloc[end, 0]:
                end += 1
            else: # End pointer is on a different stock
                # calculate the return
                d_return = (history["close"][start: end] / history["close"][start: end].shift(1)) - 1
                history["return"][start: end] = d_return
    ...
    def select_pairs(self):
        price_df = self.extract_price_data(self.universe)
        ...
        final_pairs = self.apply_selection_rules(pair_pool, self.extract_price_data(self.universe))
        return final_pairs
