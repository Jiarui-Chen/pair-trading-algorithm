...
def probabilistic_forecasting(theta, mu, sigma, beta, base_value, forecasting_length = 5, epoch = 20000, c_level = 0.95):
    all_ou = []
    for i in range(epoch):
        ...
        for i in range(1, N+1):
            x_ou[i] = x_ou[i-1] + theta*(mu - x_ou[i-1])*dt + math.sqrt(dt) * sigma*norm.rvs() #loc = mean_x3, scale = std_x3
        all_ou.append(x_ou)
