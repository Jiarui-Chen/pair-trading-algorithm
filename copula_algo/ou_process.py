...
def probabilistic_forecasting(theta, mu, sigma, beta, base_value, forecasting_length = 5, epoch = 10000, c_level = 0.95):
    all_ou = []
    for i in range(epoch):
        T = forecasting_length
        N = forecasting_length
        x_ou = [None for i in range(N+1)]
        x_ou[0] = base_value
        if T <= 1:
            dt = 1/T
        else:
        ...
        for i in range(1, N+1):
            x_ou[i] = x_ou[i-1] + theta*(mu - x_ou[i-1])*dt + sigma*norm.rvs() #loc = mean_x3, scale = std_x3
        all_ou.append(x_ou)
    ...
    return mean_forecasts.tolist(), upper_ci.tolist(), lower_ci.tolist()
